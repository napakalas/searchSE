...
class Sedmls(Sedmls):
    ...
    def validate(self):
        ...
        for counter, v in enumerate(self.data.values(), 1):
            if counter % 10 == 0: print(counter, end=' ')
    ...
    def __generatePlotImages(self, sedmlId, results, outputs):
        for outId, output in outputs.items():
            ...
            imagePdf = os.path.join(CURRENT_PATH, RESOURCE_DIR, SEDML_IMG_DIR, plotId + '.pdf')
            imagePng = os.path.join(CURRENT_PATH, RESOURCE_DIR, SEDML_IMG_DIR, plotId + '.png')

...
class Sedmls(IrCollection):
    def __init__(self, allWksDir, *paths):
        super().__init__(*paths)
        self.statusC = {'deprecated': 0, 'current': 1, 'validating': 2, 'invalid': 3}
        self.allWksDir = allWksDir
    ...
    def validate(self):
        for k, v in self.data.items():
            path = os.path.join(currentPath, self.allWksDir, v['workingDir'],v['sedml'])
            ...
            for i in range(len(issues)):
                if issues[i].startswith('Error: the imports could not be fully instantiated'):
                    issues[i] = re.sub(os.path.join(currentPath, self.allWksDir, v['workingDir'])+'/','',issues[i])
            v['issues'] = issues
    ...
    def __getSedmlData(self, k, v):
        path = os.path.join(currentPath,'workspaces',v['workingDir'],v['sedml'])
            ...
            # extract models
                ...
                for model in child:
                    if model.tag.endswith('model'):
                        ...
                        cellmlId = self.sysCellmls.getCellmlId(url=modelPath)
                        ...
                        cellml = self.sysCellmls.getCellml(id=cellmlId)
        ...
        if len(results) > 0:
            dumpPickle(results, 'resources', 'sedmlResults', str(v['id']) + '.gz')
            # generate image plot
    ...
    def __generatePlotImages(self, sedmlId, results, outputs):
        for outId, output in outputs.items():
            ...
            imagePdf = os.path.join('resources', 'sedmlImages', plotId + '.pdf')
            imagePng = os.path.join('resources', 'sedmlImages', plotId + '.png')
